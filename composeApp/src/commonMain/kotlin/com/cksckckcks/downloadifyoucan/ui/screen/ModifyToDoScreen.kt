...
class ModifyToDoScreen(
    private val todo: ToDo
): Screen {
    @Composable
    override fun Content() {
        val navigator = LocalNavigator.currentOrThrow
        val viewModel: ModifyToDoViewModel = koinInject()

        ModifyToDoScreenContent(
            todo = todo,
            viewModel = viewModel,
            onModifyClick = {
...
@Composable
fun ModifyToDoScreenContent(
    todo: ToDo,
    viewModel: ModifyToDoViewModel,
    onModifyClick: () -> Unit
) {
    LaunchedEffect(Unit) {
        viewModel.setToDo(todo = todo)
    }
